...
let countInLineOfSight asteroid =
    asteroids |> Seq.except [asteroid] |> Seq.filter (isInLineOfSight asteroid) |> Seq.length

let maxInSight = asteroids |> Seq.map (fun asteroid -> (asteroid, countInLineOfSight asteroid)) |> Seq.maxBy snd
