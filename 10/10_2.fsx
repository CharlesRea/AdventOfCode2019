...
let getOrdering (x, y): int * float =
    ...
    | true -> match y > stationY with
                | true -> (1, gradient) // Bottom right
                | false -> (0, gradient) // Top right
    | false -> match y >= stationY with
                | true -> (2, gradient) // Bottom left
                | false -> (3, gradient) // Top left

let runRotation (asteroids: (int * int) list): remainingAsteroids: (int * int) list * destroyedAsteroids: (int * int) list =
