...
let initialProgram = File.ReadAllText("./2_input.txt").Split(',') |> Seq.map int |> Seq.toArray

let rec runStep (program: array<int>, currentIndex: int) =
    match program.[currentIndex] with
    | 1 ->
        Array.set program program.[currentIndex + 3] (program.[program.[currentIndex + 1]] + program.[program.[currentIndex + 2]])
