...
let getMinimumDistance (x: Planet) (y: Planet) (orbits: Orbit list) =
    let parentsByChildren =
        orbits
        |> Seq.map (fun orbit -> (orbit.Child, orbit.Parent))
        |> Map.ofSeq
    ...
    Seq.findIndex (fun ancestor -> ancestor = firstAncestor) xAncestors
    + Seq.findIndex (fun ancestor -> ancestor = firstAncestor) yAncestors
