...
let (|PathDirection|_|) (direction: string) (path: string) =
    if path.StartsWith(direction) then Some(path.Substring(direction.Length) |> int)
    else None

let getPathCoordinates (x, y) (path: string) =
    match path with
    | PathDirection "U" length ->
        seq {
            for i in 1 .. length -> (x + i, y)
        }
    | PathDirection "R" length ->
        seq {
            for i in 1 .. length -> (x, y + i)
        }
    | PathDirection "D" length ->
        seq {
            for i in 1 .. length -> (x - i, y)
        }
    | PathDirection "L" length ->
        seq {
            for i in 1 .. length -> (x, y - i)
        }
...
let getWireCoordinates (wire: seq<string>) =
    ...
    wire
    |> Seq.fold (fun currentPath path -> Seq.append currentPath (getPathCoordinates (Seq.last currentPath) path))
           initialPath
...
let intersectionDistances =
    intersections |> Seq.map (fun point -> distanceToPoint wire1 point + distanceToPoint wire2 point)
